#!/usr/bin/env python3
"""
Emotion Timeline Analysis: Comparing emotions across different periods (Iteration 2)
- Pre-ChatGPT (2017-2021): tweets_ai_downsampled.labeled.csv
- Right after ChatGPT launch (2022): postlaunch.labeled.csv filtered for 2022
- Established ChatGPT period (2023+): postlaunch.labeled.csv filtered for 2023+
"""
...
def load_and_clean_data():
    ...
    # Load pre-ChatGPT data (tweets_ai_downsampled)
    print("1. Loading pre-ChatGPT data (tweets_ai_downsampled.labeled.csv)...")
    pre_data = pd.read_csv('Sampling_iter2/tweets_ai_downsampled.labeled.csv')
    
    # Standardize column names (handle both lowercase and capitalized)
    if 'date' in pre_data.columns:
        pre_data.rename(columns={'date': 'Date', 'tweet': 'Tweet'}, inplace=True)
    ...
    # Load postlaunch data
    print("2. Loading postlaunch data (postlaunch.labeled.csv)...")
    after_data = pd.read_csv('Sampling_iter2/postlaunch.labeled.csv')
...
def create_visualizations(all_data, results):
    ...
    plt.tight_layout()
    plt.savefig('Emotional_Analysis_iter2/emotion_timeline_analysis.png', dpi=300, bbox_inches='tight')
    
    print("Visualizations saved as 'Emotional_Analysis_iter2/emotion_timeline_analysis.png'")
...
def generate_report(results, changes, chi2, p_value):
    ...
    report = f"""
EMOTION TIMELINE ANALYSIS REPORT (ITERATION 2)
===============================================
...
1. Pre-ChatGPT Era (2017-2021): {results['Pre-ChatGPT (2017-2021)']['total']:,} tweets (from tweets_ai_downsampled.labeled.csv)
2. Right after ChatGPT Launch (2022): {results['Right after ChatGPT (2022)']['total']:,} tweets (from postlaunch.labeled.csv)
3. Established ChatGPT Era (2023+): {results['Established ChatGPT (2023+)']['total']:,} tweets (from postlaunch.labeled.csv)
...
-----------
- Data sources: 
  * tweets_ai_downsampled.labeled.csv (pre-ChatGPT, downsampled version)
  * postlaunch.labeled.csv (post-ChatGPT launch data)
...
"""
    ...
    # Save report
    with open('Emotional_Analysis_iter2/emotion_timeline_analysis_report.txt', 'w') as f:
        f.write(report)
    
    print("\nReport saved as 'Emotional_Analysis_iter2/emotion_timeline_analysis_report.txt'")
    return report

def main():
    """Main analysis function"""
    
    print("EMOTION TIMELINE ANALYSIS (ITERATION 2)")
    print("=" * 50)
    print("Comparing emotions across AI discourse time periods")
    print("Using: tweets_ai_downsampled.labeled.csv & postlaunch.labeled.csv")
    print()
    ...
    print("- Emotional_Analysis_iter2/emotion_timeline_analysis.png (visualizations)")
    print("- Emotional_Analysis_iter2/emotion_timeline_analysis_report.txt (detailed report)")
