#!/usr/bin/env python3
"""
Calculate Weighted Accuracy/Error Rates - Iteration 2
======================================================
Adjusts for class imbalance by weighting errors based on actual emotion distributions.
For iteration 2 datasets (tweets_ai_downsampled and postlaunch).
"""
...
def calculate_weighted_metrics(sample_file, evaluation_excel, labeled_full_file, dataset_name):
    ...
    total_errors = len(eval_df_clean)
...
def main():
    ...
    print("="*80)
    print("WEIGHTED ACCURACY CALCULATOR - ITERATION 2")
    print("="*80)
    ...
    # Analysis 1: Tweets AI downsampled dataset
    print("\n" + "="*80)
    print("ANALYZING: Tweets AI Downsampled Dataset (Iteration 2)")
    print("="*80)
    
    result1 = calculate_weighted_metrics(
        sample_file='Sampling_iter2/tweets_ai_sampled_400.csv',
        evaluation_excel='Evaluation_ tweets_ai.xlsx',
        labeled_full_file='Sampling_iter2/tweets_ai_downsampled.labeled.csv',
        dataset_name='Tweets AI Downsampled (Iter2)'
    )
    results.append(result1)
    
    # Analysis 2: Postlaunch dataset
    print("\n" + "="*80)
    print("ANALYZING: Postlaunch Dataset (Iteration 2)")
    print("="*80)
    
    result2 = calculate_weighted_metrics(
        sample_file='Sampling_iter2/postlaunch_sampled_400.csv',
        evaluation_excel='Evaluation_ postlaunch.xlsx',
        labeled_full_file='Sampling_iter2/postlaunch.labeled.csv',
        dataset_name='Postlaunch (Iter2)'
    )
    ...
    with open('Sample_Analysis/weighted_accuracy_report_iter2.txt', 'w') as f:
        f.write(report)
