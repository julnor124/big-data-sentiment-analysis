#!/usr/bin/env python3
"""
Create Emotion Sample Comparison - IMPROVED with Timestamp Matching
==================================================================

This script creates a sample of 400 random tweets with different emotions
from AfterChatGPT.labeled.csv, finds their original tweets using IMPROVED
timestamp matching, and creates a side-by-side comparison CSV.
"""
...
def create_emotion_sample_comparison_improved():
    """
    Create a sample of 400 tweets with different emotions using IMPROVED timestamp matching.
    """
    
    print("🎭 CREATING EMOTION SAMPLE COMPARISON - IMPROVED")
    print("=" * 70)
    print(f"Timestamp: {datetime.now()}")
    print("Using improved timestamp matching for better precision")
    print()
    ...
    output_file = 'emotion_sample_comparison_improved.csv'
    ...
    print(f"     After cleaning: {len(labeled_df):,} rows")
    ...
    orig_chatgpt_df['Date'] = pd.to_datetime(orig_chatgpt_df['Date'], errors='coerce')
    orig_chatgpt_df = orig_chatgpt_df.dropna(subset=['Date'])
    ...
    orig_genai_df['Date'] = pd.to_datetime(orig_genai_df['Date'], errors='coerce')
    orig_genai_df = orig_genai_df.dropna(subset=['Date'])
    ...
    print("\n🔍 Finding original tweets with IMPROVED timestamp matching...")
    print("   Using ±5 minute time windows for precision matching")
    ...
    for idx, row in sample_df.iterrows():
        ...
        if len(time_matches) > 0:
            ...
            for _, orig_row in time_matches.iterrows():
                ...
                similarity = calculate_text_similarity(row['Tweet'], orig_row['Tweet'])
    ...
    print(f"   - Match rate: {found_matches/len(sample_df)*100:.1f}%")
    ...
    print(f"   - Average time proximity: {comparison_df['Time_Proximity_Score'].mean():.3f}")
    ...
    print(f"\n⏱️  TIME PRECISION ANALYSIS:")
    time_ranges = [
        (0, 60, "Within 1 minute"),
        (60, 300, "1-5 minutes"),
        (300, 900, "5-15 minutes"),
        (900, float('inf'), "15+ minutes")
    ]
    
    for min_sec, max_sec, label in time_ranges:
        if max_sec == float('inf'):
            count = len(comparison_df[comparison_df['Time_Difference_Seconds'] >= min_sec])
        else:
            count = len(comparison_df[
                (comparison_df['Time_Difference_Seconds'] >= min_sec) &
                (comparison_df['Time_Difference_Seconds'] < max_sec)
            ])
        print(f"   {label}: {count} tweets ({count/len(comparison_df)*100:.1f}%)")
    ...
    for i, row in comparison_df.head(3).iterrows():
        ...
        print(f"  Time diff: {row['Time_Difference_Seconds']:.1f}s (proximity: {row['Time_Proximity_Score']:.3f})")
...
def create_high_quality_sample_improved():
    """
    Create a smaller high-quality sample for detailed analysis.
    """
    
    print("\n🔍 CREATING HIGH-QUALITY SAMPLE - IMPROVED")
    print("=" * 50)
    ...
    # Filter for high-quality matches (high similarity + time proximity)
    high_quality = comparison_df[
        (comparison_df['Similarity_Score'] > 0.3) &
        (comparison_df['Time_Proximity_Score'] > 0.5)
    ]
    ...
    sample_file = 'high_quality_emotion_sample_improved.csv'
    ...
    print(f"   - Average time proximity: {high_quality_sample['Time_Proximity_Score'].mean():.3f}")
...
def main():
    """Main function to create improved emotion sample comparison."""
    print("🎭 EMOTION SAMPLE COMPARISON TOOL - IMPROVED")
    print("=" * 60)
    ...
    comparison_df = create_emotion_sample_comparison_improved()
    if comparison_df is None:
        print("❌ Failed to create comparison dataset")
        return
    
    # Create high-quality sample
    sample_df = create_high_quality_sample_improved()
    if sample_df is None:
        print("❌ Failed to create high-quality sample")
        return
    
    print("\n" + "=" * 70)
    print("🎉 IMPROVED EMOTION SAMPLE COMPARISON COMPLETED!")
    print("=" * 70)
    print("✅ Created emotion-stratified sample")
    print("✅ Used improved timestamp matching (±5min windows)")
    print("✅ Found original tweets with high precision")
    print("✅ Created side-by-side comparison with time analysis")
    print()
    print("📁 Output files:")
    print("   - emotion_sample_comparison_improved.csv (full sample)")
    print("   - high_quality_emotion_sample_improved.csv (high-quality subset)")
    print()
    print("🔍 You can now:")
    ...
    print("   - Analyze time precision of matches")
    print("   - Study time proximity vs. content similarity")
